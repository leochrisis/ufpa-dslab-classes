...
class Inputer():
	...
	def read_vertices(self, string, grafo):
		rotulo = ''
		peso = 0.0
		estado = -1

		for palavra in self.bananasplit(string):
			if palavra == 'vertices':
				estado = 0
			elif estado == 0:
				if palavra in [',', ';']:
	...
	def read_edges(self, string, grafo):
		nome = ''
		rotulo = ''
		peso = 1.0
		aresta = ['', '']
	...
	def read_type(self, string, grafo):
		cont = 0
		start = 0
	...
	def read_from_file(self, nome_arquivo):
		grafo = Grafo(True)
		...
		if self.validate(string):
			...
			grafo = self.read_type(string, grafo)
			grafo = self.read_vertices(string, grafo)
			grafo = self.read_edges(string, grafo)
			return grafo
		else:
			return None
